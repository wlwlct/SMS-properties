...
%Intensity for main area not outliers. Overall avewav, maxave, lifetime, 
%spectrum(normalized and not normalized.) E0001 is not settled
...
%Remove sparse tail
T=histcounts(intintensity,intensityedge);
P=find(T==0);
PT=diff(P);
PC=diff(PT);
PC=find(PT==1);
Prm=intensityedge(P(PC(1,1)));
%redefine intensity edge, select the non-sparse part.
clearvars intensityedge
intensityedge=min(intintensity(:)):(Prm-min(intintensity(:)))/100:Prm;
intensityedge(1,end)=intensityedge(1,end)+1;%include the last element
%write the large intensity ones into a document
fileID=fopen([solvent ' Large Intensity.txt'],'w');
fprintf(fileID,'%4s; %3s; %10s;\n','name','sec','intensity');
[mol,sec]=find(intintensity>Prm);
na=transpose(allnames(1,mol));
for i=1:length(sec)
fprintf(fileID,'%20s; %3s; %20s;\n',na{i,1},num2str(sec(i,1)),num2str(intintensity(mol(i,1),sec(i,1))));
end
fclose(fileID);
...
try
    cd([srdir '/smaller range intensity change/']);
catch
    mkdir([srdir '/smaller range intensity change/']);
    cd([srdir '/smaller range intensity change/']);
end
...
subplot(1,2,1)
  ...
  title(['average wavelength change with small int range ' solvent])
subplot(1,2,2)
  surf(edges(1,2:end),intensityedge(1,1:end-1),normalize(intavehis,2,'range'),'EdgeColor','none');colormap(jet);view([0 0 1]);
  title(['Normalized average wavelength change with small int range ' solvent])  
saveas(gcf,[solvent ' average wavelength change with small int range.jpg']);
  saveas(gcf,[solvent ' average wavelength change with small int range.fig']);
  disp('Save average wavelength change with small int range successfully /n');
  close all
...
subplot(1,2,1)
  ...
  title(['Maximum Wavelength change with small int range ' solvent])
subplot(1,2,2)
  surf(edges(1,2:end),intensityedge(1,1:end-1),normalize(intmaxhis,2,'range'),'EdgeColor','none');colormap(jet);view([0 0 1]);
  title(['Normalized Maximum wavelength change with small int range ' solvent])
saveas(gcf,[solvent ' Normalized Maximum change with small int range.jpg']);
  saveas(gcf,[solvent ' Normalized Maximum change with small int range.fig']);
  disp('Save Normalized Maximum change with small int range successfully /n');
  close all
  
figure
subplot(1,2,1)
  surf(60:10:2500,intensityedge(1,1:end-1),intlifetimehis,'EdgeColor','none');colormap(jet);view([0 0 1]);
  title(['lifeitme change with small int range ' solvent])
subplot(1,2,2)
  surf(60:10:2500,intensityedge(1,1:end-1),normalize(intlifetimehis,2,'range'),'EdgeColor','none');colormap(jet);view([0 0 1]);
  title(['Normalized lifeitme change with small int range ' solvent])
saveas(gcf,[solvent ' lifetime change with small int range.jpg']);
  saveas(gcf,[solvent ' lifetime change with small int range.fig']);
  disp('Save lifeitme change with small int range successfully /n');
  close all
...
subplot(1,2,1)
  ...
  title(['Spectrum (add up) change with small int range ' solvent])
subplot(1,2,2)
  ...
  title(['Normalized Spectrum (add up) change with small int range ' solvent])
saveas(gcf,[solvent ' Spectrum (add up) change with small int range.jpg']);
  saveas(gcf,[solvent ' Spectrum (add up) change with small int range.fig']);
  disp('Save Spectrum (add up) change with small int range successfully /n');
  close all  
...
subplot(1,2,1)
  ...
  title(['Spectrum (normalize then add up) change with small int range ' solvent])
subplot(1,2,2)
  ...
  title(['Normalized Spectrum (normalize then add up) change with small int range ' solvent])
saveas(gcf,[solvent ' Spectrum (normalize then add up) change with small int range.jpg']);
  saveas(gcf,[solvent ' Spectrum (normalize then add up) change with small int range.fig']);
  disp('Save Spectrum (normalize then add up) change with small int range successfully /n');
  close all   
...
subplot(1,2,1)
  ...
  ylim([0 8])
  title(['Lifetime curve change with small int range ' solvent])
subplot(1,2,2)
  ...
  ylim([0 8])
  title(['Normalized lifetime curve change with small int range ' solvent])
saveas(gcf,[solvent ' Normalized lifetime curve change with small int range.jpg']);
  saveas(gcf,[solvent ' Normalized lifetime curve change with small int range.fig']);
  disp('Normalized lifetime curve change with small int range successfully /n');
  close all   
