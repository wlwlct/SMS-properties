%properties change with time, remove low intensity distribution
clearvars
solvent='F8T2N2';
% srdir=['/scratch/lwang74/PTU_spectrum_lifetime_bluehive/PTUdata/MEH/' solvent];
...
try 
  ...
  timeE0001(len_i,:)=E00sum./E01sum;
    ...
    disp([name 'may not have length of 99'])
end
end
%%
...
index=transpose(timeintensity>LowIntensity);
%histogramize
for i=1:1:99
timeavehis(i,:)=histcounts(timeave(i,index(i,:)),edges);
timeE00his(i,:)=histcounts(timeE00(index(i,:),i),edges);
timelifetimehis(i,:)=histcounts(timelifetime(i,index(i,:)),70:10:2500);
timeintensityhis(i,:)=histcounts(timeintensity(index(i,:),i),intensityedge);
timeint10000his(i,:)=histcounts(timeintensity(index(i,:),i),int10000);
timeE0001his(i,:)=histcounts(timeE0001(index(i,:),i),0:0.05:1);
end
...
try
    cd([srdir '/time change RemoveLow/']);
catch
    mkdir([srdir '/time change RemoveLow/']);
    cd([srdir '/time change RemoveLow/']);
end
