%E00 E01 ratio is related to spectrum position,  calculate index with plus
%and minus will bring some troubles
...
%Threshold_box=[0,100;101,400;401,800;801,1500;1501,3000;3001,6000;6001,7000;7001,250000];
Threshold_box=300;
...
for len_i=1:1:len
   ...
   index=(datasetfile.dataset.scatterplot.intensity(1,:)>Threshold_box);
   apdintensitycombine=cat(2,apdintensitycombine,datasetfile.dataset.scatterplot.intensity(1,index));
   ...
   o2n=1:1:99;
   opindex=double(~(index)).*o2n;
   Lifindexremove=unique(cat(2,Lifindexremove,opindex));
   Lifindexremove(Lifindexremove==0)=[];
   ...
   averagewavelengthcombine=cat(1,averagewavelengthcombine,datasetfile.dataset.scatterplot.spectrum(index,1));
  [~,place488]=min(abs(datasetfile.dataset.ccdt(:,1)-400));
    place488=1;
  onlyccd=datasetfile.dataset.ccdt(:,3:end);
  [maxvalue,maxindex]=max(onlyccd(place488:end,index),[],1);
  
  %[maxvalue,maxindex]=max(datasetfile.dataset.ccdt(place488:end,3:index+2),[],1);
  wavelengthindex=datasetfile.dataset.ccdt(maxindex,1);
  E00wavelengthcombine=cat(1,E00wavelengthcombine,wavelengthindex);
  %ratio  
  E00sum=sum(onlyccd(27:31,index),1);
  E01sum=sum(onlyccd(36:40,index),1);
  Ratio=E00sum./E01sum;
  E0001ratiocombine=cat(2,E0001ratiocombine,Ratio);
  %spectrum
  if len_i==1
      spectrum=sum(onlyccd(:,index),2);
      x=datasetfile.dataset.ccdt(:,1);
  else
  if x==datasetfile.dataset.ccdt(:,1)
  spectrum=sum(onlyccd(:,index),2)+spectrum;
  else
