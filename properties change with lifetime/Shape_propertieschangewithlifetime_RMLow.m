...
%lifetime. %E0001 is not settled;%Remove spectra and dtime based on signal
%to noise ratio.
...
for len_i=1:1:len
    ...
    try
        ...
        for ii=1:99
            clearvars A
            cd(codefolder)
            [~,A.eff_fit,~,A.numst,~]=Traceson(SecDtimespectrum(:,ii,len_i),codefolder);
            if ~isempty(A)
...
for i=1:Bad_leng
    SecDtimeShape_bin(Bad_sec(i,1),Bad_mol(i,1))=-1;
end
...
try
    cd([srdir '/lifetime change RMLow/']);
catch
    mkdir([srdir '/lifetime change RMLow/']);
    cd([srdir '/lifetime change RMLow/']);
end
